...
resource "aws_lambda_function" "draining_lambda" {
  function_name = format("%.46s-draining-function", element((regex("((([[:alnum:]]+-){3})[[:alnum:]]+)",var.autoscaling_group_name)), 0))
...
resource "aws_autoscaling_lifecycle_hook" "asg_terminate_hook" {
  name                    = format("%.47s-terminating-hook", element((regex("((([[:alnum:]]+-){3})[[:alnum:]]+)",var.autoscaling_group_name)), 0))
