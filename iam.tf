...
resource "aws_iam_role" "lambda" {
  name               = format("%.41s-draining-function-role", element((regex("((([[:alnum:]]+-){3})[[:alnum:]]+)",var.autoscaling_group_name)), 0))
...
resource "aws_iam_role_policy" "lambda_execution_policy" {
  name = format("%.39s-draining-function-policy", element((regex("((([[:alnum:]]+-){3})[[:alnum:]]+)",var.autoscaling_group_name)), 0))
...
resource "aws_iam_role" "lifecycle" {
  name               = format("%.49s-lifecycle-role", element((regex("((([[:alnum:]]+-){3})[[:alnum:]]+)",var.autoscaling_group_name)), 0))
...
resource "aws_iam_role_policy" "lifecycle_execution_policy" {
  name = format("%.47s-lifecycle-policy", element((regex("((([[:alnum:]]+-){3})[[:alnum:]]+)",var.autoscaling_group_name)), 0))
